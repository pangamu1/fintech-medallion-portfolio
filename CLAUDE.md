...
- [ADR-0026 — Unified GitHub Actions orchestrator: one scheduled master pipeline chaining ingest → bronze → silver → gold → serve via job `needs:`; reuses Databricks Jobs/Pipelines APIs + dbt Admin API + `serve_to_sheets.py`; `max-parallel:1` forced by Free-Edition ~1-concurrency; demotes `bi-refresh.yml` cron to dispatch-only](docs/adr/0026-unified-gha-orchestrator.md)
...
> **Orchestration (control plane):** the full chain runs as **one scheduled GitHub Actions workflow** — `master-pipeline.yml` (`on: schedule` Mon 06:00 UTC + `workflow_dispatch`) chains `ingest → bronze → silver → gold → serve` via job `needs:`, hard-halting on any stage failure. It reuses the existing Databricks Jobs/Pipelines REST triggers, the `prod.yml` dbt Cloud Admin-API pattern, and `serve_to_sheets.py`; fan-out stages run `max-parallel:1` (Free-Edition serverless is ~1-concurrent). GHA over Airflow (serverless, no always-on daemon). See [ADR-0026](docs/adr/0026-unified-gha-orchestrator.md). `ci.yml`/`prod.yml` stay as the untouched PR/merge dev-loop.
...
## Current Progress (last updated 2026-06-18)
...
- **Unified GHA orchestrator — Phase 11 (final build)** (`feat/gha-orchestrator` + follow-ups, PRs #38–#40, merged through 2026-06-18) — replaces the scattered per-stage triggers with **one scheduled control plane**: `.github/workflows/master-pipeline.yml` (`on: schedule` Mon 06:00 UTC + `workflow_dispatch` with a `stages` choice) chains `ingest → bronze → silver → gold → serve` via job `needs:`, hard-halting on failure. Reuses Databricks Jobs API (`run-now`+poll), Pipelines API (`updates`+poll), the `prod.yml` dbt Cloud Admin-API trigger, and `serve_to_sheets.py`; job/pipeline IDs + `FMP_API_KEY`/`ALPHA_VANTAGE_API_KEY` are TF-managed Actions `vars`/`secrets`. **Central finding:** Databricks Free Edition runs ~1 serverless workload at a time, so the Bronze (11) + Silver (5) fan-out matrices must run **`max-parallel:1`** or legs time out (PR #39). The first true end-to-end run (the orchestrator's hard-halt working) surfaced two **pre-existing Silver/Gold data defects** — `company_scd2` churn from the untracked-but-volatile FMP `description` column, and `agg_company_monthly` grouping by the SCD2 surrogate `company_key` instead of its `(symbol, year_month)` contract — both fixed in PR #40 (excluded `description`; regrouped on `(symbol, year_month)` with `max_by` attribution; full-refresh collapsed the spurious versions). Proven end-to-end: a `full` dispatch ran green in **24m35s** (ingest 5m4s · bronze 11/11 · silver 5/5 · dq 1m4s · gold 3m21s · serve 40s incl. Sheet write). `bi-refresh.yml` cron demoted to `workflow_dispatch`-only (single scheduler). Decision in [ADR-0026](docs/adr/0026-unified-gha-orchestrator.md). **Roadmap complete.**
...
### Next phase
1. **BI / dashboards (branch `feat/bi-dashboards`, merged PR #35):** Reverse-ETL serving layer LIVE (`serve_to_sheets.py` + weekly `bi-refresh.yml`; serves all 10 Gold marts → Google Sheet; pipeline merged in PRs #33–#34). Tableau Public **4-dashboard suite BUILT + PUBLISHED** (2026-06-14) at <https://public.tableau.com/app/profile/piruthviraj.a.s/viz/FinTechMedallion-MarketAnalytics/ExecutiveOverview> (Company Deep-Dive, **Executive Overview = hub/landing**, Sector & Market, Fundamentals & Valuation; + an About/Architecture page with a native dark vector diagram). Hub-and-spoke nav via Navigation button objects (no tab strip on Tableau Public); all boards standardized to a uniform **1200×1500** fixed canvas because Tableau Public renders all sheets in one shared, fixed-size viz container that clips mixed sizes (documented-reality finding, see ADR-0025). Workbook is local-only (`.twbx`), not in repo. **Two honest debts — both RESOLVED 2026-06-15** (see [ADR-0025](docs/adr/0025-bi-tableau-via-sheets-serving-layer.md) "Update — 2026-06-15"): (a) **auto-refresh activated + verified end-to-end** — Google Drive credential connected in Tableau Public web, re-published, confirmed live (a `bi-refresh` GHA run → Request Data Refresh advanced the viz's "Data refreshed" timestamp); the published viz is now a live-refreshing extract, not a static snapshot; (b) **`monthly_adj_close` added** to `agg_company_monthly.sql` + the Tableau trailing-return anchors repointed to it — though empirical check showed the trailing-1Y figures barely moved (no ticker splits within the trailing-12-month window), so the original "+126% = split-distortion" read was an over-diagnosis and the fix is correctness-by-construction, not a visible bug-squash. Decision recorded in [ADR-0025](docs/adr/0025-bi-tableau-via-sheets-serving-layer.md); single channel (Databricks native Dashboards channel cut at CP0). Plan: `~/.claude/plans/feat-bi-tableau-reverse-etl.md`.
2. **Unified orchestration via GitHub Actions (DONE — Phase 11, `feat/gha-orchestrator`, PRs #38–#40, merged 2026-06-18):** a single scheduled "master pipeline" workflow (`master-pipeline.yml`, `on: schedule:` Mon 06:00 UTC + `workflow_dispatch`) chains `ingest → bronze → silver → gold → serve` through job `needs:` dependencies, reusing the existing Databricks Jobs/Pipelines APIs + dbt Cloud Admin-API trigger + TF-managed secrets/vars. Replaces the scattered on-demand triggers with one control plane; `bi-refresh.yml` cron demoted to dispatch-only. Fan-out stages run `max-parallel:1` (Free-Edition ~1-concurrency). **Chosen over Apache Airflow deliberately:** GHA is serverless (GitHub hosts the cron + ephemeral runners — zero always-on infra), which fits the free-tier/no-ops constraint; Airflow's scheduler is an always-on daemon (cadence irrelevant to its cost) and would be over-engineering at this scale. Airflow skills belong in a separate dedicated project, not here. See [ADR-0026](docs/adr/0026-unified-gha-orchestrator.md). **This was the final roadmap build — the medallion platform is now end-to-end complete.**
